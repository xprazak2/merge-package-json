 ...
 * @param {Object} dst [description]
 * @param {Object} src [description]
 * @params {Array} sections [description]
 * @returns {Object} [description]
 */
function combine(dst, src, sections) {
	var srcSections = isEmpty(sections) ? src : pick(src, sections);

	return isEmpty(dst) ? src : assign({ }, dst, mapValues(srcSections, function (value, key) {
 ...
 */
function merge(dst, src, sections) {
	if (isString(dst) || isBuffer(dst)) {
	...
	return json.update(JSON.stringify(dst), combine(dst, src, sections), { });
}
